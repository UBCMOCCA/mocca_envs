...
class Walker3DStepperEnv(EnvBase):
    ...
    def update_steps(self):
        ...
        if self.next_step_index >= threshold:
            ...
            next = min(
                ...
                len(self.terrain_info_visual) - 1,
            )
            pos = self.terrain_info_visual[next, 0:3]
            phi, x_tilt, y_tilt = self.terrain_info_visual[next, 3:6]
            quaternion = np.array(self._p.getQuaternionFromEuler([x_tilt, y_tilt, phi]))
            self.steps[oldest].set_position(pos=pos, quat=quaternion)

    def reset_terrain_info_for_terrain_park(self, info):
        delta_z = np.sin(np.pi / 6) * np.arange(5)
        info[22:27, 2] = info[21, 2] + delta_z
        info[22:27, 4] = 0
        info[22:27, 5] = 0

        info[27, 2] = info[26, 2] - np.sin(np.pi / 8)
        info[28, 2] = info[27, 2] + np.sin(np.pi / 8)
        info[29, 2] = info[28, 2] - np.sin(np.pi / 8)
        info[30, 2] = info[29, 2] + np.sin(np.pi / 8)
        info[31, 2] = info[30, 2] - np.sin(np.pi / 8)

        delta_z = -np.sin(np.pi / 6) * np.arange(5)
        info[32:37, 2] = info[31, 2] + delta_z
        info[32:37, 4] = 0
        info[32:37, 5] = 0
        return info
    ...
    def reset(self):
        ...
        self.terrain.set_p_noise_height_field()
        self.terrain_info = self.terrain.generate_step_placements(
            self.n_steps, phi=self.base_phi
        )
        self.terrain.decorate_with_random_rocks()
        ...
        self.terrain_info = self.reset_terrain_info_for_terrain_park(self.terrain_info)

        phi_visual = self.base_phi.copy()
        # phi_visual[23] = self.base_phi[23] / 2
        phi_visual[22:26] = 0
        self.terrain_info_visual = self.terrain.generate_step_placements(
            self.n_steps, phi=phi_visual
        )
        self.terrain_info_visual = self.reset_terrain_info_for_terrain_park(
        ...
        if self.is_render:
            ...
            self._p.configureDebugVisualizer(
                shadowMapResolution=32768, shadowMapWorldSize=16
            )

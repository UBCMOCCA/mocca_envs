...
class Walker3DMocapStepperEnv(Walker3DMocapEnv):
    ...
    def __init__(self, render=False):
        ...
        high = np.inf * np.ones(
            self.robot.observation_space.shape[0] + 1 + self.lookahead * 5
        )
        self.observation_space = gym.spaces.Box(-high, high, dtype=np.float32)
        self.action_space = self.robot.action_space
    ...
    def generate_step_placements(
        ...
        # dr[0] = self.mocap_x[self.init_phase] + 0.8
        # dphi[0] = 0.0
        # dtheta[0] = np.pi / 2

        #make first step below feet
        dr[0] = self.mocap_x[self.init_phase] + 0.2
        dphi[0] = 0.0
        dtheta[0] = np.pi / 2

        # make first step slightly further to accommodate different starting poses
        dr[1] = self.mocap_x[self.init_phase] + 0.6
        dphi[1] = 0.0
        dtheta[1] = np.pi / 2
    ...
    def reset(self):
        ...
        #self.robot_state = self.robot.reset(random_pose=True)
        #self.robot.set_base_pose(pose="running_start")
        #self.robot_state = self.robot.reset(random_pose=False, pos=(0.3, 0, 1.25))
    ...
    def step(self, action):
        ...
        #print("reward", self.step_bonus + self.target_bonus - self.speed_penalty)
        #reward += self.tall_bonus - self.posture_penalty - self.joints_penalty
